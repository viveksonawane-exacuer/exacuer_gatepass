...
import { SwipeToAcceptButton } from "@/components/approvals/SwipeToAcceptButton";
...
export function PendingDecisionCard({
  ...
  if (isPending) {
    return (
      <>
        <article className={`ds-pending-card${busy ? " is-busy" : ""}`} data-status={item.status}>
          ...
          {showPendingPrimaryActions ? (
            <>
              ...
              {(onReject || onNotifyHost) ? (
                <div className="ds-pending-card__actions ds-pending-card__actions--secondary">
                  {onReject ? (
                    <button
                      type="button"
              ...
              {onApprove ? (
                <div className="ds-pending-card__swipe-wrap">
                  <SwipeToAcceptButton
                    label="Swipe to Accept"
                    disabled={decideDisabled}
                    title={decideTitle}
                    onComplete={() => onApprove(item)}
                  />
                </div>
              ) : null}
            </>
